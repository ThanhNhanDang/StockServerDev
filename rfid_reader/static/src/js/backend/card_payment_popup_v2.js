...
export class CardPaymentPopupV2 extends Component {
  static template = "rfid_reader.CardPaymentPopupV2";
  ...
  setup() {
    ...
    // Nhận props từ StatusBarButtons
    this.onScanResult = this.props.onScanResult || (() => { });
    this.onStopResult = this.props.onStopResult || (() => { });
    // Timer để tự động dừng sau 30s
    this.autoStopTimer = null;
    this.SCAN_TIMEOUT = 30000; // 30 seconds
    // Expose methods để StatusBarButtons có thể gọi
    if (this.props.exposeRef) {
      this.props.exposeRef({
  ...
  /**
   * Bắt đầu quét - được gọi từ StatusBarButtons
   */
  async startScan(type) {
    try {
      ...
      // Clear timer khi có lỗi
      this.state.isScanning = false;

      this.onScanResult(errorResult);
      return errorResult;
    }
  }
  ...
  /**
   * Dừng quét - được gọi từ StatusBarButtons
   */
  async stopScan(isAutoStop = false) {
    try {
      this.cleanup();
      this.state.operationType = "cancel_scan";
      // Logic dừng quét ở đây
      console.log("Dừng quét...");
      this.startCardOperation();

      // Logic dừng quét ở đây
      console.log(isAutoStop ? "Tự động dừng quét sau 30s..." : "Dừng quét...");

      const stopResult = {
        success: true,
        message: isAutoStop ? "Tự động dừng quét sau 30 giây" : "Đã dừng quét",
        isAutoStop: isAutoStop,
        timestamp: new Date(),
      };

      this.onStopResult(stopResult);
      ...
      this.onStopResult(errorResult);
      return errorResult;
    }
  }
  /**
   * Logic thực hiện quét (placeholder)
   */
  async performScan() {
    return new Promise((resolve) => {
      // Giả lập thời gian quét
      setTimeout(() => {
        resolve("Scan completed");
      }, 2000);
    });
  }
  ...
  startCardOperation() {
    ...
    if (this.webSocket && this.webSocket.isConnect()) {
      // Gửi lệnh đầu tiên
  ...
  startAutoStopTimer() {
    this.autoStopTimer = setTimeout(() => {
      console.log("Auto-stopping scan after 30 seconds...");
      this.stopScan(true); // true = isAutoStop
    }, this.SCAN_TIMEOUT);
  }
